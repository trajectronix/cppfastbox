...
template <c_family family> inline int my_fileno_impl(FILE *fp) noexcept
{
    if (fp == nullptr)
        return -1;
...
template <c_family family> inline void *my_fp_to_win32_handle_impl(FILE *fp) noexcept
{
    return my_get_osfile_handle(my_fileno_impl<family>(fp));
}
#endif

template <c_family family> inline int my_fclose_impl(FILE *fp) noexcept
{
...
template <c_family family> inline void my_c_io_flush_impl(FILE *fp)
{
#if defined(__NEWLIB__) && !defined(__CYGWIN__)
...
template <c_family family> inline ::fast_io::intfpos_t my_c_io_seek_impl(FILE *, ::fast_io::intfpos_t, seekdir)
{
    avr_libc_nosup_impl();
}
...
#if (defined(_WIN32) && !defined(__WINE__) && !defined(__BIONIC__)) || defined(__CYGWIN__)
    template <win32_family fam> explicit operator basic_win32_family_io_observer<fam, char_type>() const noexcept
    {
...
template <::std::integral char_type> using basic_c_file = basic_c_family_file<c_family::native, char_type>;
...
template <c_family fm, ::std::integral char_type> struct is_trivially_relocatable<basic_c_family_file<fm, char_type>>
{
